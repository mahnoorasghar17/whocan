...
const EXPLORE_ITEMS = [
  { label: 'Sellers',    Icon: UserIcon,        desc: 'Browse service providers' },
  { label: 'Buyers',     Icon: ShoppingBagIcon, desc: 'Find what you need'       },
  { label: 'Categories', Icon: GridIcon,        desc: 'All service categories'   },
];
  ...
  return (
    <header className="hero-nav" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000, padding: '12px 24px', transition: 'all 0.3s ease' }}>
      <nav style={{
        ...
        background: scrolled ? 'rgba(255,255,255,0.95)' : 'rgba(26,10,46,0.75)',
        backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)',
        borderRadius: '9999px', padding: '10px 24px',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        boxShadow: scrolled ? '0px 4px 24px rgba(165,74,255,0.12)' : '0px 4px 24px rgba(0,0,0,0.2)',
        border: scrolled ? '1px solid rgba(165,74,255,0.15)' : '1px solid rgba(255,255,255,0.1)',
            ...
            link.hasDropdown ? (
              <li key={link.label} style={{ position: 'relative' }}
                ...
                <a href={link.href} style={{
                  ...
                  padding: '8px 16px', borderRadius: '9999px', transition: 'all 0.2s ease',
                  display: 'flex', alignItems: 'center', gap: '4px',
                  background: exploreOpen ? (scrolled ? 'rgba(165,74,255,0.08)' : 'rgba(255,255,255,0.1)') : 'transparent',
                }}>
                  {link.label}
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                    style={{ transition: 'transform 0.2s ease', transform: exploreOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                    <path d="M3 4.5L6 7.5L9 4.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </a>

                {/* Dropdown — outer div at top:100% + paddingTop:8px bridges the hover gap */}
                <div style={{
                  position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)',
                  paddingTop: '8px',
                  opacity: exploreOpen ? 1 : 0,
                  pointerEvents: exploreOpen ? 'all' : 'none',
                  transition: 'opacity 0.15s ease', zIndex: 200,
                }}>
                  <div style={{
                    background: '#ffffff', borderRadius: '16px', border: '1px solid #EAECF0',
                    boxShadow: '0 8px 32px rgba(16,24,40,0.14)', minWidth: '220px', padding: '6px 0', overflow: 'hidden',
                  }}>
                    {EXPLORE_ITEMS.map((item, i) => (
                      <a key={item.label} href="/explore" style={{
                        display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px',
                        fontFamily: 'Poppins,sans-serif', fontSize: '14px', fontWeight: 500,
                        color: '#344054', textDecoration: 'none', transition: 'all 0.15s ease',
                        borderTop: i > 0 ? '1px solid #F2F4F7' : 'none',
                      }}
                        onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = '#F8F0FF'; el.style.color = '#A54AFF'; }}
                        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'transparent'; el.style.color = '#344054'; }}>
                        <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: '#F4EBFF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#A54AFF' }}>
                          <item.Icon />
                        </div>
                        <div>
              ...
              <li key={link.label}>
                <a href={link.href} style={{
                  fontFamily: 'Poppins,sans-serif', fontWeight: 500, fontSize: '14px',
                  color: scrolled ? '#344054' : 'rgba(255,255,255,0.9)',
                  padding: '8px 16px', borderRadius: '9999px', transition: 'all 0.2s ease', display: 'block',
                }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = scrolled ? 'rgba(165,74,255,0.08)' : 'rgba(255,255,255,0.1)'; el.style.color = scrolled ? '#A54AFF' : '#ffffff'; }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <a href="#app" style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 600, fontSize: '13px', color: '#101828', background: '#FEC84B', padding: '8px 16px', borderRadius: '9999px', transition: 'all 0.2s ease', whiteSpace: 'nowrap' }}
          ...
          <a href="#" style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 500, fontSize: '14px', color: scrolled ? '#344054' : 'rgba(255,255,255,0.9)', padding: '8px 14px', borderRadius: '9999px', transition: 'color 0.2s ease' }}>Log in</a>
          <a href="#" style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 600, fontSize: '14px', color: '#ffffff', background: 'linear-gradient(135deg,#BF75FF 0%,#A54AFF 50%,#8430E0 100%)', padding: '8px 20px', borderRadius: '9999px', transition: 'all 0.2s ease', boxShadow: '0 2px 12px rgba(165,74,255,0.35)' }}
